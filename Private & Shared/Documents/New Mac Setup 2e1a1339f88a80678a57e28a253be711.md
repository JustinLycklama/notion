...
1. Sign up for an Apple developer account and download the latest version of [XCode](https://developer.apple.com/download/all/?q=xcode) before installing brew.
2. [Install Homebrew](https://brew.sh/) so that you can install the other stuff you need

**Keyboard & More Trackpad Adjustments**

1. [Install middleclick by artginzburg](https://github.com/artginzburg/MiddleClick-Sonoma) so that the trackpad supports three finger middleclick.
    - `brew install --cask middleclick`
    - Run it, follow the prompt to give it access in settings, then in the top right click it to enable `Tap to click`
2. [Install Karibiner Elements](https://karabiner-elements.pqrs.org/) so that you can remap your command / control / option / global keys to match a windows keyboard layout.
    ...
    - Either- Automatically import the settings:
        - Import [this json settings file](https://github.com/m-sterspace/make-macos-sane/blob/main/configs/karabiner-elements/karabiner.json) by going to `Maintenance` > `Misc` > `Export & Import` > `Open config folder` and replacing the config file here, though note that these configs are tied to specific keyboard models so you may want to first change one key on each of your keyboards, export the config to get the model and vendor ids, and then copy them into this premade config and reimport.

**Windowing Adjustments**

1. [Install rectangle](https://rectangleapp.com/) so that windows can be snapped
    ...
    - Either- Automatically import the settings:
        - Import [this settings json file](https://github.com/m-sterspace/make-macos-sane/blob/main/configs/rectangle/RectangleConfig.json) by going to `Settings` > `Import` and selecting it.

**Monitor Control**

1. [Install BetterDisplay (free)](https://betterdisplay.pro/) or [Lunar (paid)](https://lunar.fyi/) so that you can control internal / external monitor brightness.
...
2. Install [DockDoor](https://github.com/ejbills/DockDoor) so that you can have window previews in your dock and window switcher.
...
1. [Install the Cascadia Code](https://github.com/microsoft/cascadia-code/releases) font so that you can have font ligatures.
    - Either- Manually
...
2. [Install ghostty](https://ghostty.org/) so that your terminal is GPU rendered
...
4. [Install git credential manager](https://github.com/git-ecosystem/git-credential-manager/blob/release/docs/install.md)
    - `brew install --cask git-credential-manager`
5. [Install VS Code](https://code.visualstudio.com/Download) or [VS Codium](https://vscodium.com/).
...
1. Install [NVM](https://nvm.sh/) to manage Node versions.
    - Use the installation script they provide.

**Java**

1. Install [sdkman](https://sdkman.io/) to manage Java versions.
    - Use the installation script they provide.

**Ruby**

1. Install [ruby-build](https://github.com/rbenv/ruby-build) and [rbenv](https://github.com/rbenv/rbenv) to manage Ruby versions.
